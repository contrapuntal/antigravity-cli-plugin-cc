...
description: Use when the user asks Antigravity to challenge a code change, design assumption, tradeoff, or reason a change should not ship in Codex.
---
...
Invoke the companion's `adversarial-review` command. Forward explicit `--base <ref>` and `--model <display name>`. Pass any focus text as one safely quoted positional argument after `--`, preserving its literal content. Include `--` even when focus is the only argument so the dispatcher's legacy single-string parser does not reinterpret it.

Keep the existing Git target selection when no base is supplied. Do not add `--write`. The dispatcher embeds the diff in the prompt and runs agy in a temporary working directory; this is review intent, not enforced filesystem isolation. Host filesystem policy supplies restrictions.

Return Antigravity's challenge findings faithfully, including its reasoning about assumptions and tradeoffs.
